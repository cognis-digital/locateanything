...
**locateanything** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `locateanything` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
locateanything ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
locateanything ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
locateanything ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
```
...
works whatever `locateanything` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="locateanything") for rec in records]   # records = your JSON output
