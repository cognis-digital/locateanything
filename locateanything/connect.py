"""Native cognis-connect emit for locateanything — forward findings to any platform.

Maps locateanything's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    locateanything ... --format json | locateanything-emit --to stix
    locateanything-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out.pop('exif_gps', None)
        out.pop('candidates', None)
        out.pop('note', None)
        out['title'] = rec.get('place', '')
        out['severity'] = 'info'
        out['type'] = 'geolocation'
        out['description'] = rec.get('rationale', '')
        out['tags'] = [rec.get('country'), rec.get('region'), rec.get('city')] or []
        out['ipv4'] = ''
        out['domain'] = ''
        out['url'] = ''
        out['sha256'] = ''
