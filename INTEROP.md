...
How **locateanything** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `locateanything` lives in the **Geospatial / GEOINT** cluster.
...
| from | relation | to |
|---|---|---|
| `locateanything` | JSON findings compose with | [`geolens`](https://github.com/cognis-digital/geolens), [`geoaoi-pro`](https://github.com/cognis-digital/geoaoi-pro) |
| `locateanything` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `locateanything` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `locateanything` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the geospatial / geoint cluster.** `locateanything` output feeds the next tool:
```bash
locateanything ... --format json > out.json          # this tool's findings (see `locateanything --help`)
geolens ... < out.json                      # the cluster sibling consumes them
```
...
locateanything ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
locateanything ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
locateanything ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `locateanything`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `locateanything` slots
